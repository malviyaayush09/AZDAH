...
export const FAQS = [
  ...
  { q: 'What if I miss a class I booked?', a: 'Cancellations must be told to us at least 6 hours before the class starts, and the class still counts against your pack. You also get one reschedule a month, and that can be used any time before the class begins — so if something comes up late, move the class rather than miss it. There are no refunds or extensions once a class pack has been purchased. Class packs are valid for four weeks, with no transfers and no extensions beyond four weeks. If you are unsure about your timings, we urge you to take a smaller class pack and buy only for the classes you know you can be fully present for.' },
